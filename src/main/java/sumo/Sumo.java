...
import sumo.arena.handlers.BungeeHandler;
import sumo.arena.handlers.SoundHandler;
import sumo.arena.handlers.VaultHandler;
import sumo.commands.AutoTabCompleter;
import sumo.commands.ConsoleCommands;
import sumo.commands.GameCommands;
import sumo.commands.setup.SetupCommandsHandler;
import sumo.commands.setup.SetupTabCompleter;
import sumo.datahandler.ArenasManager;
import sumo.datahandler.PlayerDataStore;
import sumo.datahandler.ScoreboardManager;
import sumo.eventhandler.HeadsPlusHandler;
import sumo.eventhandler.MenuHandler;
import sumo.eventhandler.PlayerLeaveArenaChecker;
import sumo.eventhandler.PlayerStatusHandler;
import sumo.eventhandler.RestrictionHandler;
import sumo.eventhandler.ShopHandler;
import sumo.kits.Kits;
import sumo.lobby.GlobalLobby;
import sumo.menu.Menus;
import sumo.messages.Language;
import sumo.messages.Messages;
import sumo.parties.Parties;
import sumo.signs.SignHandler;
import sumo.signs.editor.SignEditor;
import sumo.utils.Bars;
import sumo.utils.Shop;
import sumo.utils.Sounds;
import sumo.utils.Stats;
import sumo.utils.TitleMsg;
import sumo.utils.Utils;
...
public class Sumo extends JavaPlugin {
	...
	private static final String SPIGOT_URL = "https://www.spigotmc.org/resources/Sumo_reloaded." + SPIGOT_ID + "/";
	...
	private void setupPlugin() {
		getCommand("sumo").setExecutor(new GameCommands(this));
		getCommand("sumosetup").setExecutor(new SetupCommandsHandler(this));
		getCommand("sumoconsole").setExecutor(new ConsoleCommands(this));
		getCommand("sumo").setTabCompleter(new AutoTabCompleter());
		getCommand("sumosetup").setTabCompleter(new SetupTabCompleter());
		...
		if (PlaceholderAPI != null && PlaceholderAPI.isEnabled()) {
			...
			new SumoPlaceholders(this).register();
		}
