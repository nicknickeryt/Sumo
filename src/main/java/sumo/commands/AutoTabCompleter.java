...
public class AutoTabCompleter implements TabCompleter {
	...
	public List<String> onTabComplete(CommandSender sender, Command cmd, String alias, String[] args) {
		if (cmd.getName().equalsIgnoreCase("sumo") || cmd.getName().equalsIgnoreCase("sp")) {
			...
			if (args.length == 1) {
				...
				if (sender.hasPermission("sumo.spectate")) {
					list.add("spectate");
				}
				if (sender.hasPermission("sumo.listrewards")) {
				...
				if (Stream.of("invite", "unkick", "accept", "decline").anyMatch(s -> s.equalsIgnoreCase(args[1]))) {
					...
					list.addAll(Sumo.getInstance().getParties().getPartyMembers(sender.getName()));
				}
			}
			for (String s : list) {
