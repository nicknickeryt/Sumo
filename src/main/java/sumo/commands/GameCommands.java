...
public class GameCommands implements CommandExecutor {
	...
	public GameCommands(Sumo plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		// join arena
			...
			if (args.length == 1 && player.hasPermission("sumo.joinmenu")) {
		...
		// listrewards
			...
			if (!player.hasPermission("sumo.listrewards")) {
				Messages.sendMessage(player, Messages.nopermission);
				return false;
			}
			if (args.length != 2) {
		...
		// start
			...
			if (!arena.getStatusManager().isArenaStarting()) {
				plugin.getServer().getConsoleSender().sendMessage("[Sumo_reloaded] Arena " + ChatColor.GOLD + arena.getArenaName() + ChatColor.WHITE + " force-started by " + ChatColor.AQUA + player.getName());
		...
		// party
			...
			if (!player.hasPermission("sumo.party")) {
				Messages.sendMessage(player, Messages.nopermission);
				return false;
			}
			if (!plugin.isParties()) {
