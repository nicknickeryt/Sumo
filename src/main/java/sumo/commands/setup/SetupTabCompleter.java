...
public class SetupTabCompleter implements TabCompleter {
	...
	private static final List<String> ARENA_COMMANDS = Arrays.asList("setarena", "setloselevel", "setspawn", "addspawn", "setspectate", "setwaitingspawn", "finish",
			"deletespectate", "deletespawnpoints", "setregenerationdelay", "setmaxplayers", "setminplayers", "setvotepercent",
			"settimelimit", "setcountdown", "setmoneyreward", "setteleport", "enable", "disable", "setreward", "enablekits", "disablekits", "linkkit",
			"unlinkkit", "delete", "setdamage", "setfee", "setcurrency", "configure", "deletewaitingspawn");
	...
	public List<String> onTabComplete(CommandSender sender, Command cmd, String alias, String[] args) {
		...
		if (!sender.hasPermission("sumo.setup")) {
			return null;
		}
		...
		if (args.length == 1) {
			...
			if (ARENA_COMMANDS.contains(args[0])) {
				list.addAll(Sumo.getInstance().amanager.getArenasNames());
				...
				list.addAll(Sumo.getInstance().getKitManager().getKits());

			} else if (args[0].equalsIgnoreCase("setlanguage")) {
				list.addAll(Sumo.getInstance().getLanguage().getTranslatedLanguages());
			}
			...
			if (args[0].equalsIgnoreCase("setteleport")) {
				...
				Arena arena = Sumo.getInstance().amanager.getArenaByName(args[1]);
				if (arena != null) {
