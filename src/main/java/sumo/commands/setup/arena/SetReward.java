...
package sumo.commands.setup.arena;
...
import sumo.conversation.ConversationType;
import sumo.conversation.SumoConversation;
import sumo.messages.Messages;

public class SetReward implements CommandHandlerInterface {
	
	private Sumo plugin;
	public SetReward(Sumo plugin) {
		this.plugin = plugin;
	}
	@Override
	public boolean handleCommand(Player player, String[] args) {
		...
		if (arena != null) {
			...
			new SumoConversation(plugin, player, arena, ConversationType.ARENAPRIZE).begin();
